             ...
             <CardContent className="p-0">
               ...
               <div className="hidden md:block">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {data.videosList?.map((v: any) => (
                        <tr key={v.id} className="border-t">
                          <td className="px-6 py-4">{v.profiles?.email}</td>
                          <td className="px-6 py-4">
                            <span className="text-xs uppercase font-bold px-2 py-1 bg-muted rounded">{v.status}</span>
                          </td>
                 ...
                 {data.videosList?.map((v: any) => (
                    <div key={v.id} className="p-4 space-y-3">
                      <div className="flex justify-between">
                        <span className="text-xs text-muted-foreground">{v.profiles?.email}</span>
                        <span className="text-xs uppercase font-bold">{v.status}</span>
                      </div>
                      <div className="flex gap-2">
                         {(v.status === 'processing' || v.status === 'pending') && (
                            <form action={(fd) => handleAction(refundVideo, fd)} className="flex-1">
                               <input type="hidden" name="videoId" value={v.id} />
                               <input type="hidden" name="userId" value={v.user_id} />
                               <Button size="sm" variant="outline" className="w-full text-orange-600">Refund</Button>
                            </form>
                         )}
                         <form action={(fd) => handleAction(deleteVideo, fd)} className="flex-1">
                            <input type="hidden" name="videoId" value={v.id} />
                            <Button size="sm" variant="ghost" className="w-full text-red-500 border">Delete</Button>
                         </form>
                      </div>
                    </div>
                 ))}
